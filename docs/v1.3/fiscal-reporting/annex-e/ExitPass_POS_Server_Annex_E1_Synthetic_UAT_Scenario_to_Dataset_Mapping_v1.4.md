# ExitPass POS Server Annex E-1 Synthetic UAT Scenario-to-Dataset Mapping v1.4

## 1. Control

Every included case is `SPECIFICATION_ONLY`, `IMPLEMENTATION_NOT_AUTHORIZED`, `LOADING_NOT_AUTHORIZED`, and `EXECUTION_NOT_AUTHORIZED`. Identity names remain under the v1.1 identity grammar. Version 1.4 preserves SP-A13 monetary values and changes only hash-relevant parking-session reference serialization: each applicable command uses the lowercase `D` text of the same deterministic UUID carried by its statutory snapshot. Arithmetic, counts, identity names, UUID values, and journal facts do not change.
