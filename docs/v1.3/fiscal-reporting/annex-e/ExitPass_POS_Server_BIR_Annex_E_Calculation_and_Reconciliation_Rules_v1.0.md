...
| --- | --- | --- | --- | --- | --- |
| Gross sales D07 | Z gross sales | Qualifying recorded Sales Invoices only; nonnegative current model | Exact equality to Z | `EXISTING_EXITPASS_DECISION` | Supported |
| Net sales D27 | Z net sales | Approved VAT-inclusive final fiscal amount for qualifying Sales Invoices | Exact equality to current GTA contribution under Z-007 approved contract | `EXISTING_EXITPASS_DECISION` | Supported |
...
The second and third equations are **decision-required** because official field numbers and the 32 physical columns do not align one-to-one after the expanded statutory discount group. This package does not reinterpret them as conventional accounting equations. AE-DR-006 must approve the mapping before D26 VAT Payable or D27 validation is coded.
...
| AE-RC-006 | D19 = approved D12:D18 composition | Annex E deduction components | 0 | `RECOMMENDED_FOR_APPROVAL`; AE-DR-006 | Block until approved |
