...
`BLOCKED` means the scenario cannot become an executable acceptance test until the cited AE-DR decision is approved.

## 2. Scenarios

| ID | Precondition / authoritative input | Action | Expected output and validation | Safe error | Audit / mutation | Readiness |
| --- | --- | --- | --- | --- | --- | --- |
| AE-AC-001 | Closed period, committed Z, complete approved profile, normal VATable facts | Generate E-1 | Exact H01:H10 and D01:D32; all reconciliations exact | None | `annex_e_generated`; no fiscal mutation | `BLOCKED` by AE-DR-001:010,019:024 |
| AE-AC-002 | Committed empty-period Z | Generate | Approved zero/no-activity representation; equal GTA; advanced Z | `annex_e_contract_unresolved` until approval | Safe failure/none | `BLOCKED` AE-DR-021 |
...
| AE-AC-007 | Cancellation classification exists but no approved E-1 mapping | Generate | No file | `annex_e_unsupported_classification` | Denial/none | `BLOCKED` AE-DR-015 |
| AE-AC-008 | Nonzero refund source | Generate | No file; do not fold into Returns | `annex_e_unsupported_classification` | Denial/none | `BLOCKED` AE-DR-015 |
| AE-AC-009 | Nonzero return source | Generate | No file until sign/period/VAT mapping approved | `annex_e_unsupported_classification` | Denial/none | `BLOCKED` AE-DR-015 |
| AE-AC-010 | Nonzero adjustment source | Generate | No file; no mapping to Others/Remarks | `annex_e_unsupported_classification` | Denial/none | `BLOCKED` AE-DR-015 |
...
| AE-AC-012 | Training transaction in source | Generate | No file until training rule approved | `annex_e_unsupported_classification` | Denial/none | `BLOCKED` AE-DR-023 |
| AE-AC-013 | Classified range gap | Generate | Approved gap representation only | Contract unresolved until mapping approved | Denial/none | `BLOCKED` AE-DR-022 |
| AE-AC-014 | Unexplained sequence gap | Generate | No file | `annex_e_unexplained_gap` | Denial/none | Source support ready |
| AE-AC-015 | Stored Z/profile misses mandatory field | Generate | No partial file | `annex_e_mandatory_field_missing` | Failure reference/none | Ready after AE-DR-005 |
...
| AE-AC-019 | Same operation and semantics | Retry | Byte-identical output, filename, hash | None | Replay event/none | `BLOCKED` AE-DR-017/018 |
...
| AE-AC-024 | Existing successful export for same Z/profile, different operation | Generate | Behavior follows approved duplicate/lineage rule | Contract unresolved | None or new metadata only | `BLOCKED` AE-DR-017/018 |
...
| AE-AC-026 | Approved post-close correction request | Generate correction | Immutable new lineage or reject, per approval | Contract unresolved | No prior mutation | `BLOCKED` AE-DR-017 |
...
| AE-AC-031 | Output filename components contain unsafe characters | Generate | Deterministic sanitization after approval; no traversal | None or invalid profile | Safe audit/none | `BLOCKED` AE-DR-004 |
...
| AE-AC-038 | Multiple fiscal ranges/series in one Z | Generate | Apply approved row/sheet rule or fail | `annex_e_range_ambiguous` | Failure/none | `BLOCKED` AE-DR-003/022 |
...
- Assert every `UNRESOLVED` reference resolves to this decision register.
