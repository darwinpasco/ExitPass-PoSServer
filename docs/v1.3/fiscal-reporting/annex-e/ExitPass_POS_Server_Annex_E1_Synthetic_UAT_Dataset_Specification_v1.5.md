# ExitPass POS Server Annex E-1 Synthetic UAT Dataset Specification v1.5

## 1. Document control

| Item | Value |
|---|---|
| Specification ID | `pos-server-annex-e1-synthetic-uat-dataset-specification:v1.5` |
| Status | `READY_FOR_V1_5_SPECIFICATION_AUTHORIZATION_REVIEW` |
...
Version 1.5 is a prospective documentation correction limited to findings IR14-01, IR14-02, and E14-01 from the merged v1.4 authorization review. Versions 1.0 through 1.4 and their authorization reviews remain immutable historical records. A separate independent v1.5 authorization review is required before the canonical implementation gate can change.

## 2. Purpose and boundary

This package preserves independently verified M01 package-root calculation, M02 statutory parking-session identity, and M04 workbook/case-package recursion while correcting only IR14-01, IR14-02, and E14-01. It resolves M03 on the package-author side by publishing one definitive 1,690-row authorization-critical semantic registry. It specifies, but does not create, machine-readable artifacts for the same 19 included scenarios. It does not authorize runtime, API, database, migration, configuration, dependency, CI, environment, loading, UAT, workbook-generation, evidence-acceptance, delivery, BIR-submission, or Production work.

The complete construction authority is the ordered combination of:

1. this specification;
2. the v1.5 scenario mapping and expected-value matrix;
3. the v1.5 source-population matrix;
4. the v1.5 canonical row population;
5. the v1.5 canonical source-row and hash contract;
6. the v1.2 148-row Electronic Journal ledger incorporated by exact hash in the v1.5 hash contract;
7. the v1.5 AE1H family schemas, deterministic instance ranges, and single authorization-critical 1,690-row semantic-preimage registry in the canonical row population.
...
UUIDv5 uses RFC 4122 network-order namespace bytes and UTF-8 NFC name bytes. Scenario width is three and ordinal width is four. The registry remains 2,364 names and 2,364 unique UUIDv5 values. Version 1.5 changes no identity name or UUID.
...
The v1.5 hash contract preserves AE1H framing and defines exact binary framing for offline source rows, evidence rows, workbook content, case packages, and the specification package root. Runtime-shaped fiscal-document, workbook-request, Accounting-fact, Electronic Journal semantic, and Electronic Journal integrity hashes continue to use their current source canonicalizers. Every family identifies included and excluded fields, ordering, null/omission behavior, predecessor behavior, and digest output.

The v1.5 manifest excludes itself. It hashes exact repository bytes for the other eight v1.5 members and frames their descriptors under `annex-e1-specification-package-root:sha256:v1.5`. The hash contract fixes path encoding, descriptor member order, integer endianness, binary digest representation, member ordering, and the complete root preimage. The manifest publishes the final byte lengths, member digests, exact root-preimage Base64, root-preimage byte length, and final root digest.
...
For each Senior Citizen and PWD document, v1.5 uses the tax-inclusive pre-discount amount as the statutory snapshot original amount:
...
The v1.5 graph is directed from a digest to the bytes it hashes. Its topological order is: fixed literals and UUIDs; canonical source rows; source-row semantic hashes; workbook canonical content bytes; workbook content hash; workbook row semantic hash; remaining evidence-row hashes; case-package digest; specification-member file digests; specification package root. A workbook row contains `workbook_content_sha256`, never a case-package digest. A case-package descriptor contains the completed workbook-row semantic hash. No child row contains an ancestor digest and every self-hash field is excluded from its own preimage. Cycle detection over the declared node and edge registry returns zero cycles.

The exact graph, exclusions, node framing, and worked case `DS-AE1-001` are normative in the v1.5 hash contract. Case-package bytes are future machine-readable artifacts and are not generated by this documentation task.

## 10. Reconciliation rules

R01 through R12 remain the v1.2 equations. The v1.5 source population supplies exact row identities and hashes for R08 and current-runtime-valid statutory inputs for R11/R12. Tolerance is zero monetary units and zero rows.
...
Package readiness is `READY_FOR_V1_5_SPECIFICATION_AUTHORIZATION_REVIEW`. IR14-01 and IR14-02 are resolved and E14-01 is corrected on the package-author side; M01, M02, and M04 remain preserved; M03 is author-side resolved by the complete registry. Canonical implementation remains `BLOCKED_ANNEX_E1_SYNTHETIC_DATASET_IMPLEMENTATION`. Only a separate independent v1.5 authorization review may change that gate.
