# ExitPass POS Server Annex E-1 Synthetic UAT Dataset Specification v1.3

## 1. Document control

| Item | Value |
|---|---|
| Specification ID | `pos-server-annex-e1-synthetic-uat-dataset-specification:v1.3` |
| Status | `READY_FOR_V1_3_SPECIFICATION_AUTHORIZATION_REVIEW` |
| Repository baseline | `cfb9d0c49912bd2efdc5185bdc9725b7c1c936d9` |
...
Version 1.3 is a prospective documentation correction. Versions 1.0, 1.1, 1.2, and their authorization reviews remain immutable historical records. A separate independent v1.3 authorization review is required before the canonical implementation gate can change.

## 2. Purpose and boundary

This package closes the v1.2 review findings for non-empty hashing, SP-A finality, and canonical row population. It specifies, but does not create, machine-readable artifacts for the same 19 included scenarios. It does not authorize runtime, API, database, migration, configuration, dependency, CI, environment, loading, UAT, workbook-generation, evidence-acceptance, delivery, BIR-submission, or Production work.

The complete construction authority is the ordered combination of:

1. this specification;
2. the v1.3 scenario mapping and expected-value matrix;
3. the v1.3 source-population matrix;
4. the v1.3 canonical row population;
5. the v1.3 canonical source-row and hash contract;
6. the v1.2 148-row Electronic Journal ledger incorporated by exact hash in the v1.3 hash contract.
...
The v1.3 hash contract defines exact binary framing for offline source rows, evidence rows, case packages, and the package root. Runtime-shaped fiscal-document, workbook-request, Accounting-fact, Electronic Journal semantic, and Electronic Journal integrity hashes continue to use their current source canonicalizers. Every family identifies included and excluded fields, ordering, null/omission behavior, predecessor behavior, and digest output.

The v1.3 manifest excludes itself. It hashes exact repository bytes for the other eight v1.3 members, ordered by UTF-8 relative path, and then frames those member descriptors under `annex-e1-specification-package-root:sha256:v1.3`.
...
For each Senior Citizen and PWD document, v1.3 uses the tax-inclusive pre-discount amount as the statutory snapshot original amount:
...
SP-A13 Annex values remain D07 41200, D19 16200, D27 21400, and D29 22100. Correcting the statutory snapshot original amount changes source/request hashes but does not change report arithmetic, identities, row counts, or EJ fact payloads.

## 9. Reconciliation rules

R01 through R12 remain the v1.2 equations. The v1.3 source population supplies exact row identities and hashes for R08 and current-runtime-valid statutory inputs for R11/R12. Tolerance is zero monetary units and zero rows.
...
## 10. Governed population and journal
...
Package readiness is `READY_FOR_V1_3_SPECIFICATION_AUTHORIZATION_REVIEW`. Canonical implementation remains `BLOCKED_ANNEX_E1_SYNTHETIC_DATASET_IMPLEMENTATION` until a separate independent review is merged.
