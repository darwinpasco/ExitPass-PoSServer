# ExitPass POS Server Annex E-1 Synthetic UAT Dataset Specification v1.6

## 1. Document control

| Item | Value |
|---|---|
| Specification ID | `pos-server-annex-e1-synthetic-uat-dataset-specification:v1.6` |
| Status | `READY_FOR_REVIEW` |
| Repository baseline | `2761a41b5ba7d4d5482d2973238238666425fa16` |
...
| Canonical implementation authority | v1.5 authorization plus the operator-approved v1.6 arithmetic correction |
| Dataset or validator implementation | Exact offline v1.6 artifacts implemented in this branch |

Version 1.6 is the minimum prospective correction to the independently authorized v1.5 package. It corrects only the confirmed SP-A F20 arithmetic contradiction and its deterministic hash descendants while preserving versions 1.0 through 1.5 as immutable historical records. The same explicitly authorized task implements the corrected machine-readable dataset and offline validator; no additional specification authorization review is required by this package.

## 2. Purpose and boundary

This package is the minimum prospective correction to the authorized v1.5 package. It corrects the fifteen SP-A F20 rows whose registry values incorrectly used active gross for D07 and then subtracted the void twice in R01. It preserves every historical v1.5 file, identity, count, timestamp, source input, Electronic Journal row, and unaffected semantic tuple. The same bounded task creates the exact v1.6 machine-readable dataset and offline validator; it does not authorize runtime, API, database, migration, configuration, dependency, CI, environment, loading, UAT, workbook generation, evidence acceptance, delivery, BIR submission, or Production work.

The complete construction authority is the ordered combination of:

1. this specification;
2. the v1.6 scenario mapping and expected-value matrix;
3. the v1.6 source-population matrix;
4. the v1.6 canonical row population;
5. the v1.6 canonical source-row and hash contract;
6. the v1.2 148-row Electronic Journal ledger incorporated by exact hash in the v1.6 hash contract;
7. the v1.6 AE1H family schemas, deterministic instance ranges, and single authorization-critical 1,690-row semantic-preimage registry in the canonical row population.
...
UUIDv5 uses RFC 4122 network-order namespace bytes and UTF-8 NFC name bytes. Scenario width is three and ordinal width is four. The registry remains 2,364 names and 2,364 unique UUIDv5 values. Version 1.6 changes no identity name or UUID.
...
The v1.6 hash contract preserves AE1H framing and defines exact binary framing for offline source rows, evidence rows, workbook content, case packages, and the specification package root. Runtime-shaped fiscal-document, workbook-request, Accounting-fact, Electronic Journal semantic, and Electronic Journal integrity hashes continue to use their current source canonicalizers. Every family identifies included and excluded fields, ordering, null/omission behavior, predecessor behavior, and digest output.

The v1.6 manifest excludes itself. It hashes exact repository bytes for the other eight v1.6 members and frames their descriptors under `annex-e1-specification-package-root:sha256:v1.6`. The hash contract fixes path encoding, descriptor member order, integer endianness, binary digest representation, member ordering, and the complete root preimage. The manifest publishes the final byte lengths, member digests, exact root-preimage Base64, root-preimage byte length, and final root digest.
...
For each Senior Citizen and PWD document, v1.6 uses the tax-inclusive pre-discount amount as the statutory snapshot original amount:
...
SP-A13 Annex values are D07 41200, D19 16200, D27 21400, and D29 22100. For each of the fifteen affected rows, R01 is `41200 - 0 - 11200 = 30000` and R06 is `21400 + 16200 + 3600 = 41200`. Version 1.5 incorrectly published D07 30000, R01 18800, and R06 30000 in the definitive semantic registry even though its expected-value matrix and current calculation engine required the corrected values. Version 1.6 recalculates the fifteen F20 calculation and row hashes, affected workbook aggregates/content/rows, replay and recovery evidence, workbook audits, case-package digests, governed member hashes, and package root. It does not change UUID-generation names, source hashes, row counts, or EJ fact payloads.

## 9. Acyclic artifact-hash graph

The v1.6 graph is directed from a digest to the bytes it hashes. Its topological order is: fixed literals and UUIDs; canonical source rows; source-row semantic hashes; workbook canonical content bytes; workbook content hash; workbook row semantic hash; remaining evidence-row hashes; case-package digest; specification-member file digests; specification package root. A workbook row contains `workbook_content_sha256`, never a case-package digest. A case-package descriptor contains the completed workbook-row semantic hash. No child row contains an ancestor digest and every self-hash field is excluded from its own preimage. Cycle detection over the declared node and edge registry returns zero cycles.

The exact graph, exclusions, node framing, and worked case `DS-AE1-001` are normative in the v1.6 hash contract. The machine-readable dataset includes all nineteen completed case-package preimages and digests; the offline validator independently reconstructs and compares them with Appendix B of the canonical population.

## 10. Reconciliation rules

R01 through R12 remain the v1.2 equations. The v1.6 source population supplies exact row identities and hashes for R08 and current-runtime-valid statutory inputs for R11/R12. Tolerance is zero monetary units and zero rows.
...
Package and implementation readiness is `READY_FOR_REVIEW`. IR14-01 and IR14-02 remain resolved, E14-01 remains corrected, M01, M02, and M04 remain preserved, and M03 remains closed by the complete registry. The corrected v1.6 JSONL dataset and deterministic offline validator are part of the same bounded implementation. Loading, runtime changes, Controlled UAT, workbook generation, delivery, BIR submission, and Production remain unauthorized.
