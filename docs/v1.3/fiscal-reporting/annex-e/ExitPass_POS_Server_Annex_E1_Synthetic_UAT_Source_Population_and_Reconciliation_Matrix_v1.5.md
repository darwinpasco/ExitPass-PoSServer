# ExitPass POS Server Annex E-1 Synthetic UAT Source Population and Reconciliation Matrix v1.5

## 1. Normative expansion

A row is the unique product of scenario, registered object token and ordinal, the v1.5 canonical row contract, its profile, and deterministic chronology. The canonical population document supplies every field value or closed expression. Missing bindings fail validation; database defaults and runtime-generated values are not substituted.
...
For every SC/PWD document, `CentralPmsParkingSessionRef` is the lowercase RFC 4122 `D` text of the exact UUID stored as `ParkingSessionId`. Case 001 document 0001 uses `eb706d21-9161-5083-a4a2-6f703b62845a` in both positions. Across 15 A13 cases, 30 comparisons are required, 30 match, and 0 mismatch. The prior `SYN-AE1-PARKING-NNN-OOOO` alias is not a valid runtime value for this field.
...
| R08 | all copied IDs and v1.5 hashes exact | exact | exact |
...
## 8. Hash dependency order

The deterministic order is source values, source-row hashes, EJ chains, workbook content, workbook-row hash, evidence hashes, case-package digest, exact Markdown member hashes, and package root. Workbook content excludes its own digest and every ancestor digest. Workbook rows contain only the workbook-content digest and length. Replay, conflict, and recovery evidence never contains the case-package digest. Case packages consume completed child hashes. Graph validation reports zero cycles.

## 9. Counts and v1.4 delta

| Family group | v1.4 | v1.5 | Delta |
|---|---:|---:|---:|
