# ExitPass POS Server Annex E-1 Synthetic UAT Dataset Specification v1.4

## 1. Document control

| Item | Value |
|---|---|
| Specification ID | `pos-server-annex-e1-synthetic-uat-dataset-specification:v1.4` |
| Status | `READY_FOR_V1_4_SPECIFICATION_AUTHORIZATION_REVIEW` |
...
Version 1.4 is a prospective documentation correction limited to findings M01-M04 from the merged v1.3 authorization review. Versions 1.0 through 1.3 and their authorization reviews remain immutable historical records. A separate independent v1.4 authorization review is required before the canonical implementation gate can change.

## 2. Purpose and boundary

This package author-side resolves only M01 package-root calculation, M02 statutory parking-session identity, M03 AE1H instance binding, and M04 workbook/case-package recursion. It specifies, but does not create, machine-readable artifacts for the same 19 included scenarios. It does not authorize runtime, API, database, migration, configuration, dependency, CI, environment, loading, UAT, workbook-generation, evidence-acceptance, delivery, BIR-submission, or Production work.

The complete construction authority is the ordered combination of:

1. this specification;
2. the v1.4 scenario mapping and expected-value matrix;
3. the v1.4 source-population matrix;
4. the v1.4 canonical row population;
5. the v1.4 canonical source-row and hash contract;
6. the v1.2 148-row Electronic Journal ledger incorporated by exact hash in the v1.4 hash contract;
7. the v1.4 AE1H family schemas, deterministic instance ranges, informational binding registry, and authorization-critical semantic-preimage registry in the canonical row population.
...
UUIDv5 uses RFC 4122 network-order namespace bytes and UTF-8 NFC name bytes. Scenario width is three and ordinal width is four. The registry remains 2,364 names and 2,364 unique UUIDv5 values. Version 1.4 changes no identity name or UUID.
...
The v1.4 hash contract preserves AE1H framing and defines exact binary framing for offline source rows, evidence rows, workbook content, case packages, and the specification package root. Runtime-shaped fiscal-document, workbook-request, Accounting-fact, Electronic Journal semantic, and Electronic Journal integrity hashes continue to use their current source canonicalizers. Every family identifies included and excluded fields, ordering, null/omission behavior, predecessor behavior, and digest output.

The v1.4 manifest excludes itself. It hashes exact repository bytes for the other eight v1.4 members and frames their descriptors under `annex-e1-specification-package-root:sha256:v1.4`. The hash contract fixes path encoding, descriptor member order, integer endianness, binary digest representation, member ordering, and the complete root preimage. The manifest publishes the final byte lengths, member digests, exact root-preimage Base64, root-preimage byte length, and final root digest.
...
The same service normalizes `CentralPmsParkingSessionRef` and compares it with `ParkingSessionId.Value.ToString("D")` using `StringComparison.OrdinalIgnoreCase`. Version 1.4 therefore serializes deterministic `REF(parking-session,o)` in lowercase RFC 4122 `D` form in both fields. For case 001 document 0001, both are exactly `eb706d21-9161-5083-a4a2-6f703b62845a`; `SYN-AE1-PARKING-001-0001` is forbidden for this field. The rule applies to all 30 statutory snapshots and their 30 fiscal-document commands. Author-side comparison: 30 checked, 30 equal, 0 mismatches.

For each Senior Citizen and PWD document, v1.4 uses the tax-inclusive pre-discount amount as the statutory snapshot original amount:
...
SP-A13 Annex values remain D07 41200, D19 16200, D27 21400, and D29 22100. Correcting the parking-session reference changes affected fiscal-document request hashes, statutory-row source hashes, dependent audit hashes, workbook content hashes, case-package digests, and the specification package root. It does not change UUID-generation names, report arithmetic, row counts, or EJ fact payloads.

## 9. Acyclic artifact-hash graph

The v1.4 graph is directed from a digest to the bytes it hashes. Its topological order is: fixed literals and UUIDs; canonical source rows; source-row semantic hashes; workbook canonical content bytes; workbook content hash; workbook row semantic hash; remaining evidence-row hashes; case-package digest; specification-member file digests; specification package root. A workbook row contains `workbook_content_sha256`, never a case-package digest. A case-package descriptor contains the completed workbook-row semantic hash. No child row contains an ancestor digest and every self-hash field is excluded from its own preimage. Cycle detection over the declared node and edge registry returns zero cycles.

The exact graph, exclusions, node framing, and worked case `DS-AE1-001` are normative in the v1.4 hash contract. Case-package bytes are future machine-readable artifacts and are not generated by this documentation task.

## 10. Reconciliation rules

R01 through R12 remain the v1.2 equations. The v1.4 source population supplies exact row identities and hashes for R08 and current-runtime-valid statutory inputs for R11/R12. Tolerance is zero monetary units and zero rows.
...
The 29 family schemas expand to exactly 1,690 instances. Appendix A retains the 1,690 informational identity-to-schema binding digests. Appendix B separately publishes exact ordered AE1H value tuples, complete semantic-preimage bytes, byte lengths, and expected semantic SHA-256 values for the historical 840 blocked instances. Those 840 comprise 687 M03 field-binding instances in the 11 affected families, 97 M02 runtime-identity instances, and 56 M04 workbook/evidence instances. Independent Windows PowerShell 5.1 reconstruction from the Appendix B tuples produced 840/840 byte-for-byte and digest matches. Combined with the 850 instances independently closed by the v1.3 review, author-side closure is 1,690/1,690 with zero blocked instances.
...
Package readiness is `READY_FOR_V1_4_SPECIFICATION_AUTHORIZATION_REVIEW`. M01-M04 are resolved on the package-author side, but canonical implementation remains `BLOCKED_ANNEX_E1_SYNTHETIC_DATASET_IMPLEMENTATION`. Only a separate independent v1.4 authorization review may change that gate.
