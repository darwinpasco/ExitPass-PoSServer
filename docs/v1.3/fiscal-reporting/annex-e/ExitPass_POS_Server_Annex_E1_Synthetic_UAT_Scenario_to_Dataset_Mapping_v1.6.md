# ExitPass POS Server Annex E-1 Synthetic UAT Scenario-to-Dataset Mapping v1.6

## 1. Control

Every included case is `OFFLINE_DATASET_IMPLEMENTED`, `LOADING_NOT_AUTHORIZED`, and `EXECUTION_NOT_AUTHORIZED`. Identity names remain under the v1.1 identity grammar. Version 1.6 preserves the authorized parking-session reference serialization: each applicable command uses the lowercase `D` text of the same deterministic UUID carried by its statutory snapshot. It corrects the confirmed SP-A F20 arithmetic and deterministic descendants without changing counts, identity names, UUID values, timestamps, or journal facts.
