# ExitPass POS Server Annex E-1 Synthetic UAT Scenario-to-Dataset Mapping v1.3

## 1. Control

Every included case is `SPECIFICATION_ONLY`, `IMPLEMENTATION_NOT_AUTHORIZED`, `LOADING_NOT_AUTHORIZED`, and `EXECUTION_NOT_AUTHORIZED`. Identity names remain under the v1.1 identity grammar. `SP-A13` differs from SP-A12 only in the statutory snapshot original amount and resulting source/request hashes; arithmetic, counts, identities, and journal facts do not change.
